...
export async function POST(request: Request) {
  try {
    ...
    const sql = db();
    const found = await sql`SELECT id,public_id,user_id,event_slug,owner_name,category_name,status,used_at FROM tickets WHERE qr_token=${token} LIMIT 1`;
    ...
    const updated = await sql`
      ...
      RETURNING id,public_id,user_id,event_slug,owner_name,category_name,status,used_at
    `;
    if (!updated[0]) {
      const latest = await sql`SELECT id,public_id,user_id,event_slug,owner_name,category_name,status,used_at FROM tickets WHERE id=${ticket.id}`;
    ...
    await updateUserLoyaltyByVisits(String(updated[0].user_id));
