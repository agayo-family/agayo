...
import type { EventPalette } from '@/lib/event-palette';
...
type Editor = {
  ...
  description:string; secondaryDescription:string; eventRules:string; posterImage:string; status:'draft'|'published'|'cancelled'; salesState:'open'|'closed'|'coming-soon'; ticketMode:'general-admission'|'zones'|'seats';
...
const blank = (): Editor => ({
  title:'', date:'', start:'18:00', end:'21:00', age:'14+', city:'Йошкар-Ола', venue:'', address:'', description:'', secondaryDescription:'', eventRules:DEFAULT_EVENT_RULES, posterImage:'', status:'draft', salesState:'coming-soon', ticketMode:'zones', themePrimary:'#220708', themeSecondary:'#751013', themeAccent:'#e12622',
...
function eventViewHref(slug:string,status:string){ return `/events/${encodeURIComponent(slug)}${status==='published'?'':'?preview=admin'}`; }
function safeColor(value:string,fallback:string){ return /^#[0-9a-f]{6}$/i.test(value.trim())?value.trim():fallback; }
...
export default function AdminEventsManager({ access, previewMode, events, setEvents }: { access:Access; previewMode:boolean; events:StoredEvent[]; setEvents:(events:StoredEvent[])=>void }) {
  ...
  function patchPalette(key:'themePrimary'|'themeSecondary'|'themeAccent',value:string){ patch(key,value); setPaletteStatus('Палитра скорректирована вручную. Сохрани мероприятие, чтобы применить её на публичной странице.'); }
  ...
  async function openEdit(item:StoredEvent){
    setPosterPreview('');setPaletteStatus('Сохранённая палитра события. Новая афиша пересчитает её автоматически.');setAutoPalette(null);
    if(previewMode){
  ...
  async function analyzePosterPalette(file:File){
    try{
      ...
      setPaletteStatus('Автопалитра определена по афише и адаптирована под фирменную основу AGAYO. При необходимости цвета можно поправить вручную.');
    }catch(error){
  ...
  async function save(targetStatus?:'draft'|'published'){
    ...
    try{
      ...
      const payload={
        ...
        description:editor.description,secondaryDescription:editor.secondaryDescription,eventRules:editor.eventRules,posterImage,status:targetStatus||editor.status,
  ...
  if(editor){ return <div className="admin-event-workspace">
    <div className="admin-workspace-bar"><button className="admin-secondary" type="button" onClick={()=>{setEditor(null);setStats(null);setPosterPreview('');setRulesOpen(false);setMessage('')}}>← К событиям</button><div><span>{editor.id?'РЕДАКТИРОВАНИЕ':'НОВОЕ СОБЫТИЕ'}</span><strong>{editor.slug||'ещё без адреса'}</strong></div><div className="admin-workspace-actions"><button className="admin-secondary" type="button" onClick={()=>setRulesOpen(v=>!v)}>{rulesOpen?'Закрыть правила':'Правила мероприятия'}</button><button className="admin-secondary" disabled={saving} onClick={()=>void save('draft')}>Сохранить</button>{can('publish_events')?<button className="admin-primary" disabled={saving} onClick={()=>void save('published')}>{saving?'СОХРАНЯЕМ…':'Опубликовать'}</button>:null}</div></div>
    {rulesOpen?<section className="admin-event-rules-panel">
      <div className="admin-section-heading"><span>R</span><div><b>ПРАВИЛА МЕРОПРИЯТИЯ</b><small>Этот текст увидит покупатель перед оплатой. Правила сохраняются отдельно для каждого события и становятся частью условий покупки билета.</small></div></div>
      <textarea value={editor.eventRules} onChange={e=>patch('eventRules',e.target.value)} placeholder="Напиши правила конкретного мероприятия…" rows={16}/>
      <div className="admin-rules-actions"><button className="admin-secondary" type="button" onClick={()=>patch('eventRules',DEFAULT_EVENT_RULES)}>Вернуть базовые правила AGAYO</button>{editor.slug?<a className="admin-secondary admin-button-link" href={`/events/${encodeURIComponent(editor.slug)}/rules${editor.status==='published'?'':'?preview=admin'}`} target="_blank" rel="noreferrer">Открыть как гость</a>:<span className="admin-rules-hint">Сначала сохрани событие — после этого появится публичная ссылка на правила.</span>}</div>
    </section>:null}
    <div className="admin-editor admin-event-editor-v2">
      ...
      <div className="admin-editor-section"><div className="admin-section-heading"><span>02</span><div><b>АФИША + АВТОПАЛИТРА</b><small>Загрузи афишу — AGAYO сам выделит её основные цвета и построит из них оформление публичной страницы.</small></div></div>{(posterPreview||editor.posterImage)?<div className="admin-current-poster"><img src={posterPreview||editor.posterImage} alt="Текущая афиша"/><span>{posterPreview?'Новая афиша выбрана':'Текущая афиша'}</span></div>:null}<label className="admin-upload"><input id="admin-event-poster" type="file" accept="image/*" onChange={async e=>{const file=e.currentTarget.files?.[0];if(!file)return;if(file.size>12*1024*1024){setMessage('Афиша больше 12 МБ. Выбери файл поменьше.');e.currentTarget.value='';return;}await analyzePosterPalette(file);if(previewMode){try{const url=await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||''));reader.onerror=()=>reject(new Error('Не удалось прочитать афишу'));reader.readAsDataURL(file);});patch('posterImage',url);setPosterPreview('');setMessage('Афиша выбрана, автопалитра рассчитана. Нажми «Сохранить изменения».');}catch(err){setMessage(err instanceof Error?err.message:'Ошибка афиши');}}else{setPosterPreview(URL.createObjectURL(file));setMessage('Новая афиша выбрана, палитра страницы пересчитана автоматически. Афиша загрузится при сохранении.');}}}/><strong>＋ ЗАМЕНИТЬ АФИШУ</strong><small>JPG / PNG / WEBP · цвета страницы определяются автоматически</small></label><div className="admin-theme-preview"><div><span>ПАЛИТРА СТРАНИЦЫ</span><small>{paletteStatus}</small></div><div className="admin-swatches" aria-label="Цветовая палитра мероприятия"><i style={{background:safeColor(editor.themePrimary,'#0B0B0C')}}/><i style={{background:safeColor(editor.themeSecondary,'#6B1F2B')}}/><i style={{background:safeColor(editor.themeAccent,'#C21F39')}}/></div></div><div className="admin-palette-controls"><label><span>ОСНОВНОЙ ФОН</span><div><input type="color" value={safeColor(editor.themePrimary,'#0B0B0C')} onChange={e=>patchPalette('themePrimary',e.target.value)}/><input value={editor.themePrimary} onChange={e=>patchPalette('themePrimary',e.target.value)} maxLength={7}/></div></label><label><span>ВТОРИЧНЫЙ ЦВЕТ</span><div><input type="color" value={safeColor(editor.themeSecondary,'#6B1F2B')} onChange={e=>patchPalette('themeSecondary',e.target.value)}/><input value={editor.themeSecondary} onChange={e=>patchPalette('themeSecondary',e.target.value)} maxLength={7}/></div></label><label><span>АКЦЕНТ</span><div><input type="color" value={safeColor(editor.themeAccent,'#C21F39')} onChange={e=>patchPalette('themeAccent',e.target.value)}/><input value={editor.themeAccent} onChange={e=>patchPalette('themeAccent',e.target.value)} maxLength={7}/></div></label></div><div className="admin-palette-note"><span>КАК ЭТО РАБОТАЕТ</span><p>Афиша задаёт характер страницы, а Almost Black, Graphite, Burgundy и Accent Red AGAYO удерживают фирменный стиль. Автопалитра — стартовая точка, не ограничение.</p>{autoPalette?<button className="admin-secondary" type="button" onClick={()=>{patch('themePrimary',autoPalette.primary);patch('themeSecondary',autoPalette.secondary);patch('themeAccent',autoPalette.accent);setPaletteStatus('Возвращена автоматически рассчитанная палитра.')}}>Вернуть автопалитру</button>:null}</div></div>
