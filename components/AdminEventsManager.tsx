...
export default function AdminEventsManager({ access, previewMode, events, setEvents }: { access:Access; previewMode:boolean; events:StoredEvent[]; setEvents:(events:StoredEvent[])=>void }) {
  const [editor,setEditor]=useState<Editor|null>(null); const [saving,setSaving]=useState(false); const [loading,setLoading]=useState(false); const [message,setMessage]=useState(''); const [stats,setStats]=useState<Stats|null>(null); const [statsLoading,setStatsLoading]=useState(false); const [posterPreview,setPosterPreview]=useState('');
  ...
  if(editor){ return <div className="admin-event-workspace">
    ...
    <div className="admin-editor admin-event-editor-v2">
      ...
      <div className="admin-editor-section"><div className="admin-section-heading"><span>02</span><div><b>АФИША</b><small>Можно оставить текущую или загрузить новую.</small></div></div>{(posterPreview||editor.posterImage)?<div className="admin-current-poster"><img src={posterPreview||editor.posterImage} alt="Текущая афиша"/><span>{posterPreview?'Новая афиша выбрана':'Текущая афиша'}</span></div>:null}<label className="admin-upload"><input id="admin-event-poster" type="file" accept="image/*" onChange={async e=>{const file=e.currentTarget.files?.[0];if(!file)return;if(file.size>12*1024*1024){setMessage('Афиша больше 12 МБ. Выбери файл поменьше.');e.currentTarget.value='';return;}if(previewMode){try{const url=await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||''));reader.onerror=()=>reject(new Error('Не удалось прочитать афишу'));reader.readAsDataURL(file);});patch('posterImage',url);setPosterPreview('');setMessage('Афиша выбрана. Нажми «Сохранить изменения», чтобы она осталась после обновления страницы.');}catch(err){setMessage(err instanceof Error?err.message:'Ошибка афиши');}}else{setPosterPreview(URL.createObjectURL(file));setMessage('Новая афиша выбрана. Она загрузится при сохранении. Если Blob ещё не подключён, остальные изменения всё равно сохранятся.');}}}/><strong>＋ ЗАМЕНИТЬ АФИШУ</strong><small>JPG / PNG / WEBP</small></label></div>
      ...
      <div className="admin-editor-actions"><a className="admin-secondary admin-button-link" href={editor.slug?`/events/${editor.slug}`:'#'} target="_blank" rel="noreferrer">Открыть страницу</a><button className="admin-secondary" disabled={saving} onClick={()=>void save()}>Сохранить изменения</button>{can('publish_events')?<button className="admin-primary" disabled={saving} onClick={()=>void save('published')}>{saving?'СОХРАНЯЕМ…':'СОХРАНИТЬ И ОПУБЛИКОВАТЬ'}</button>:null}</div>{message?<p className="admin-event-message">{message}</p>:null}
    </div>
  </div> }

  return <div className="admin-events-v2">
    <div className="admin-events-heading"><div><span>МЕРОПРИЯТИЯ</span><h2>УПРАВЛЕНИЕ<br/>СОБЫТИЯМИ</h2></div>{canCreate?<button className="admin-primary" type="button" onClick={()=>{setEditor(blank());setStats(null);setPosterPreview('');setMessage('')}}>＋ Создать событие</button>:null}</div>
